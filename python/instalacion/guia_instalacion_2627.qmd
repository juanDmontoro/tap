...
El fichero **`tap.yml`** es la "lista de la compra": Python 3.11 y todas las librerías del curso (scikit-learn, XGBoost, LightGBM, SHAP, Optuna, DoubleML, TensorFlow/Keras, pandas, matplotlib, plotly…). Conda lee la lista, descarga las versiones compatibles y las instala **aisladas** en un entorno llamado `tap`. **Es el mismo fichero para Windows, Mac y Linux.**
...
**3.2 Abre la terminal de conda** (Windows: **Anaconda Prompt (miniconda3)** · Mac: **Terminal**) y **entra en la carpeta Descargas** con `cd Downloads`. Después lista su contenido para confirmar que `tap.yml` está ahí:
...
Escribe `cd ` (con un espacio) y **arrastra la carpeta** desde el Explorador / Finder hasta la ventana de la terminal: se pega la ruta sola. Pulsa <kbd>Enter</kbd>.
:::

**3.3 Crea el entorno** con esta línea (tarda entre 5 y 20 minutos según la conexión; **no cierres la ventana**):
...
**3.4 Comprueba.** Activa el entorno y mira qué Python responde:
...
(En Mac el prompt es `(tap) tu-mac Downloads %`; los comandos son los mismos.)
