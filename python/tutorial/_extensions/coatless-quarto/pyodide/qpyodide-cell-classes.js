...
class InteractiveCell extends BaseCell {
    ...
    async runCode(code) {
        ...
        // Nullify the output areas of content
        this.outputCodeDiv.innerHTML = "";
        this.outputGraphDiv.innerHTML = "";

        // Generate a new figure element and attach it BEFORE running the code:
        // the matplotlib backend paints via document.getElementById(), which only
        // finds the canvas if the figure is already in the document. Attaching
        // afterwards left blank figures in Firefox/Safari (timer ordering).
        ...
        if (/\S/.test(result)) {
            ...
            div.textContent = result;   // texto plano: evita que salidas como <class 'int'> se interpreten como HTML
            pre.appendChild(div);
        } else {
